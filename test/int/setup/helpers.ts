...
export const testHelpers = {
    ...
    createMessage: async (
        teamId: string,
        chatId: string,
        authorChatMemberId: string | null,
        overrides?: {
            message?: string;
        ...
        const isFromAiAgent = overrides?.isFromAiAgent ?? false;
        const resolvedAuthorId = isFromAiAgent ? null : authorChatMemberId;
        ...
        if (!teamsWithRealTenantDb.has(teamId)) {
            return {
                ...
                isFromAiAgent,
                visibleToMemberId: overrides?.visibleToMemberId ?? null,
            };
        }
        const tenant = getTenantPrisma(teamId);
