...
function Lp(e, t = Xs) {
    ...
    if (e < 0)
        throw new be(xe.EncodeTimeNegative, `Time must be positive: ${e}`);
                    ...
                    let l = this.#m(a),
                        c = await this.#d(l, r, () =>
                            r
                                .queryRaw(l)
                                .catch((u) =>
                                    t.args.type === "rawSql" ? yn(u) : Ve(u)
                                )
                        );
                    s === void 0
                        ? (s = c)
                        : (s.rows.push(...c.rows),
                          (s.lastInsertId = c.lastInsertId));
                }
                return {
            ...
            case "join": {
                ...
                let a = await Promise.all(
                    t.args.children.map(async (l) => ({
                        joinExpr: l,
                        childRecords: (
                            await this.interpretNode(l.child, r, n, i)
                        ).value,
                    }))
                );
...
${n.backtrace}`,
    ...
    async request(t, { interactiveTransaction: r, customDataProxyFetch: n }) {
        ...
        try {
            a = this.#p(() =>
                this.#m({ queries: [t], execute: () => s.compile(i) })
            );
        } catch (l) {
            throw this.#u(l);
        }
...
var Xn =
        ...
        (r) => {
            ...
            if (ar(r))
                ...
                let [o, ...s] = r;
                ((n = o),
                    (i = { values: Ge(s || []), __prismaRawParameters__: !0 }));
            } else
                switch (t) {
                    case "sqlite":
                    case "mysql": {
